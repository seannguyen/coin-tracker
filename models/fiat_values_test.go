...
var (
	fiatValueDBTypes = map[string]string{`AmountCents`: `bigint`, `BalanceID`: `integer`, `CreatedAt`: `timestamp without time zone`, `Currency`: `character varying`, `ID`: `integer`, `UpdatedAt`: `timestamp without time zone`}
	_                = bytes.MinRead
)
