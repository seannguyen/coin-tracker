...
var (
	balanceDBTypes = map[string]string{`Amount`: `numeric`, `CreatedAt`: `timestamp without time zone`, `Currency`: `character varying`, `ID`: `integer`, `SnapshotID`: `integer`, `UpdatedAt`: `timestamp without time zone`}
	_              = bytes.MinRead
)
